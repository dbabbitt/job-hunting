...
class HeaderCategories(object):
    ...
    def __init__(self, cu=None, verbose=False):
        ...
        if not len(self.MINIMUM_QUALIFICATION_HEADERS_LIST):
            if s.pickle_exists('MINIMUM_QUALIFICATION_HEADERS_LIST'):
                self.MINIMUM_QUALIFICATION_HEADERS_LIST = s.load_object('MINIMUM_QUALIFICATION_HEADERS_LIST')
            else:
                ...
                s.store_objects(MINIMUM_QUALIFICATION_HEADERS_LIST=self.MINIMUM_QUALIFICATION_HEADERS_LIST, verbose=verbose)
        ...
        if not len(self.PREFERRED_QUALIFICATION_HEADERS_LIST):
            if s.pickle_exists('PREFERRED_QUALIFICATION_HEADERS_LIST'):
                self.PREFERRED_QUALIFICATION_HEADERS_LIST = s.load_object('PREFERRED_QUALIFICATION_HEADERS_LIST')
            else:
                ...
                s.store_objects(PREFERRED_QUALIFICATION_HEADERS_LIST=self.PREFERRED_QUALIFICATION_HEADERS_LIST, verbose=verbose)
        ...
        if not len(self.LEGAL_NOTIFICATION_HEADERS_LIST):
            if s.pickle_exists('LEGAL_NOTIFICATION_HEADERS_LIST'):
                self.LEGAL_NOTIFICATION_HEADERS_LIST = s.load_object('LEGAL_NOTIFICATION_HEADERS_LIST')
            else:
                ...
                s.store_objects(LEGAL_NOTIFICATION_HEADERS_LIST=self.LEGAL_NOTIFICATION_HEADERS_LIST, verbose=verbose)
        ...
        if not len(self.OFFICE_LOCATION_HEADERS_LIST):
            if s.pickle_exists('OFFICE_LOCATION_HEADERS_LIST'):
                self.OFFICE_LOCATION_HEADERS_LIST = s.load_object('OFFICE_LOCATION_HEADERS_LIST')
            else:
                ...
                s.store_objects(OFFICE_LOCATION_HEADERS_LIST=self.OFFICE_LOCATION_HEADERS_LIST, verbose=verbose)
        ...
        if not len(self.SUPPLEMENTAL_PAY_HEADERS_LIST):
            if s.pickle_exists('SUPPLEMENTAL_PAY_HEADERS_LIST'):
                self.SUPPLEMENTAL_PAY_HEADERS_LIST = s.load_object('SUPPLEMENTAL_PAY_HEADERS_LIST')
            else:
                ...
                s.store_objects(SUPPLEMENTAL_PAY_HEADERS_LIST=self.SUPPLEMENTAL_PAY_HEADERS_LIST, verbose=verbose)
        ...
        if not len(self.EDUCATIONAL_REQUIREMENT_HEADERS_LIST):
            if s.pickle_exists('EDUCATIONAL_REQUIREMENT_HEADERS_LIST'):
                self.EDUCATIONAL_REQUIREMENT_HEADERS_LIST = s.load_object('EDUCATIONAL_REQUIREMENT_HEADERS_LIST')
            else:
                self.EDUCATIONAL_REQUIREMENT_HEADERS_LIST = ['<p>Education:</p>', '<b>Education</b>']
                s.store_objects(EDUCATIONAL_REQUIREMENT_HEADERS_LIST=self.EDUCATIONAL_REQUIREMENT_HEADERS_LIST, verbose=verbose)
        ...
        if not len(self.INTERVIEW_PROCEDURE_HEADERS_LIST):
            if s.pickle_exists('INTERVIEW_PROCEDURE_HEADERS_LIST'):
                self.INTERVIEW_PROCEDURE_HEADERS_LIST = s.load_object('INTERVIEW_PROCEDURE_HEADERS_LIST')
            else:
                self.INTERVIEW_PROCEDURE_HEADERS_LIST = ['<p>COVID-19 Precaution(s):</p>']
                s.store_objects(INTERVIEW_PROCEDURE_HEADERS_LIST=self.INTERVIEW_PROCEDURE_HEADERS_LIST, verbose=verbose)
        ...
        if not len(self.CORPORATE_SCOPE_HEADERS_LIST):
            if s.pickle_exists('CORPORATE_SCOPE_HEADERS_LIST'):
                self.CORPORATE_SCOPE_HEADERS_LIST = s.load_object('CORPORATE_SCOPE_HEADERS_LIST')
            else:
                self.CORPORATE_SCOPE_HEADERS_LIST = ['<b>Careers with Optum.</b>', '<b>Why LPL?</b>', '<b>Information on Interviews:</b>', "<p>Company's website:</p>", '<b>Patients First | Innovation | Winning Culture | Heart Recovery</b>', '<b>Cogito Business Intelligence Developer, Enterprise Data &amp; Digital Health</b>']
                s.store_objects(CORPORATE_SCOPE_HEADERS_LIST=self.CORPORATE_SCOPE_HEADERS_LIST, verbose=verbose)
        ...
        if not len(self.POSTING_DATE_HEADERS_LIST):
            if s.pickle_exists('POSTING_DATE_HEADERS_LIST'):
                self.POSTING_DATE_HEADERS_LIST = s.load_object('POSTING_DATE_HEADERS_LIST')
            else:
                self.POSTING_DATE_HEADERS_LIST = ['<b>Job Posting :</b>']
                s.store_objects(POSTING_DATE_HEADERS_LIST=self.POSTING_DATE_HEADERS_LIST, verbose=verbose)
    ...
    def append_parts_of_speech_list(self, navigable_parent, pos_list=[]):
        for column_name, symbol_suffix in self.cu.subtypes_dict.items():
    ...
    def get_pos_symbol(
        ...
        for subtype in self.cu.subtypes_list:
            exec(f"{subtype} = params_dict.get('{subtype}')")
        
        params_list = [is_header] + self.cu.subtypes_list
    ...
    def get_feature_tuple(
        ...
        if type(is_header) == bool:
            if is_header:
                symbol_prefix = 'H-'
            if all(feature_dict.get(param) is None for param in self.cu.subtypes_list):
                try_to_guess(default_symbol=f'{symbol_prefix}O')
            elif all(feature_dict.get(param, False) is False for param in self.cu.subtypes_list):
                try_to_guess(default_symbol=f'{symbol_prefix}O')
            else:
                for column_name, symbol_suffix in self.cu.subtypes_dict.items():
