...
class SqlUtilities(object):
    ...
    def __init__(self):
        ...
        self.select_navigable_parent_by_file_name_sql_str = """
            ...
            FROM
                [Jobhunting].[dbo].[NavigableParentSequence] nu INNER JOIN
                [Jobhunting].[dbo].[NavigableParents] t ON
                nu.[navigable_parent_id] = t.[navigable_parent_id] INNER JOIN
                [Jobhunting].[dbo].[FileNames] f ON
                nu.[file_name_id] = f.[file_name_id]
            WHERE RTRIM(f.[file_name]) = ?
            ORDER BY nu.[sequence_order] ASC;"""
        ...
        self.select_query_sql_str = """
            SELECT RTRIM(nu.[file_name]) AS file_name
            FROM HeaderTagSequence nu
                INNER JOIN #query q
                ON
                    q.header_tag = nu.header_tag AND
                    q.sequence_order = nu.sequence_order
            GROUP BY nu.file_name
    ...
    def populate_headertags_table(self, db, verbose=False):
        ...
        document_body_elements_set = set(['a','abbr','acronym','address','applet','area','article','aside','audio','b','bdi','bdo','big','blockquote','br','button','canvas','caption','center','cite','code','col','colgroup','data','datalist','dd','del','dfn','dir','div','dl','dt','em','embed','fieldset','figcaption','figure','font','footer','form','h1','h2','h3','h4','h5','h6','header','hr','i','img','input','ins','isindex','kbd','keygen','label','legend','li','main','map','mark','menu','meter','nav','noscript','object','ol','optgroup','option','output','p','param','pre','progress','q','rb','rp','rt','rtc','ruby','nu','samp','script','section','select','small','source','span','strike','strong','sub','sup','table','tbody','td','template','textarea','tfoot','th','thead','time','tr','track','tt','u','ul','var','video','wbr'])
        ...
        inline_elements_set = set(['a','abbr','acronym','b','bdi','bdo','big','br','cite','code','data','del','dfn','em','font','i','ins','kbd','mark','q','rb','rp','rt','rtc','ruby','nu','samp','script','small','span','strike','strong','sub','sup','template','time','tt','u','var','wbr'])
        anchor_set = set(['a'])
        phrase_elements_set = set(['abbr','acronym','b','big','code','dfn','em','font','i','kbd','nu','samp','small','strike','strong','tt','u','var'])
        general_set = set(['abbr','acronym','dfn','em','strong'])
        computer_phrase_elements_set = set(['code','kbd','samp','var'])
        presentation_set = set(['b','big','font','i','nu','small','strike','tt','u'])
        span_set = set(['span'])
    ...
    def find_basic_quals_section(self, cursor, child_strs_list, ea=None):
        if ea is None: ea = html_analysis.ElementAnalysis()
