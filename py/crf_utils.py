...
class CrfUtilities(object):
    ...
    def __init__(
        self, ha=None, hc=None, cu=None, lru=None, slrcu=None,
        scrfcu=None, ssgdcu=None, verbose=False
    ):
        self.hc = hc
        self.slrcu = slrcu
        self.scrfcu = scrfcu
        ...
        if self.slrcu and hasattr(self.slrcu, 'ha'):
            self.ha = self.slrcu.ha
        elif self.scrfcu and hasattr(self.scrfcu, 'ha'):
            self.ha = self.scrfcu.ha
        elif self.ssgdcu and hasattr(self.ssgdcu, 'ha'):
            self.ha = self.ssgdcu.ha
        ...
        if self.hc and hasattr(self.hc, 'cu'):
            self.cu = self.hc.cu
        elif self.slrcu and hasattr(self.slrcu, 'cu'):
            self.cu = self.slrcu.cu
        elif self.scrfcu and hasattr(self.scrfcu, 'cu'):
            self.cu = self.scrfcu.cu
        elif self.ssgdcu and hasattr(self.ssgdcu, 'cu'):
            self.cu = self.ssgdcu.cu
        else:
            self.cu = cu
    ...
    def build_pos_conditional_random_field_elements(self, verbose=False):
        ...
        if self.s and self.s.pickle_exists('crf_CRF'):
            self.CRF = self.s.load_object('crf_CRF')
        else:
            if self.s and self.s.pickle_exists('HEADER_PATTERN_DICT'):
    ...
    def retrain_pos_classifier(self, header_pattern_dict=None, verbose=False):
        ...
        # elif self.is_flask_running():
            # pos_sgd_predict_single = self.get_pos_sgd_predict_single_from_api
        else:
        ...
        try:
            ...
            raise
        self.s.store_objects(crf_CRF=self.CRF, verbose=verbose)
    ...
    def word2features(self, feature_tuples_list, i):
        ...
        for es in self.cu.elements_sets_list:
            feature_key = f'tag.{es}'
            # if feature_key in compiled_fdl:
            var_set = eval(f'self.cu.{es}')
            features.update({
        ...
        if hasattr(self.slrcu, 'pos_predict_percent_fit_dict'):
            features.update({
                'child_str.pos_lr_predict_single': self.slrcu.predict_single(child_str),
            })
        # elif self.is_flask_running():
            # features.update({
        ...
        # Seek a SectionCRFClassifierUtilities object
        if self.scrfcu and hasattr(self.scrfcu, 'pos_symbol_crf'):
            features.update({
                'child_str.pos_crf_predict_single': self.scrfcu.predict_single(child_str),
            })
        # elif self.is_flask_running():
            # features.update({
        ...
        if self.ssgdcu:
            features.update({
                'child_str.pos_sgd_predict_single': self.ssgdcu.predict_single(
                    child_str
                ),
            })
        # elif self.is_flask_running():
            # features.update({
                # 'child_str.pos_sgd_predict_single': self.get_pos_sgd_predict_single_from_api(
                    # child_str
                # ),
            # })
        if i == 0:
