...
################################################
## SGDClassifier is-corporate-scope functions ##
################################################
class IsCorporateScopeSgdClassifier(PosSymbolSgdClassifier):
    ...
    def prepare_training_data(self, verbose=False):
        ...
        df.is_corporate_scope = df.is_corporate_scope.map(
            lambda x: {'True': 1, 'False': 0, True: 1, False: 0}.get(x, x)
        )
        if verbose:
            print(
                f'I have {df.shape[0]:,} hand-labeled',
                'corporate scope htmls prepared'
            )
        train_data_list = df.navigable_parent.tolist()
        train_labels_list = df.is_corporate_scope.values
