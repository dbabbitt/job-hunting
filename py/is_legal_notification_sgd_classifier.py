...
###################################################
## SGDClassifier is-legal-notification functions ##
###################################################
class IsLegalNotificationSgdClassifier(PosSymbolSgdClassifier):
    ...
    def prepare_training_data(self, verbose=False):
        ...
        df.is_legal_notification = df.is_legal_notification.map(
            lambda x: {true: 1, false: 0, True: 1, False: 0}.get(x, x)
        )
        if verbose:
            print(
                f'I have {df.shape[0]:,} hand-labeled',
                'legal notification htmls prepared'
            )
        train_data_list = df.navigable_parent.tolist()
        train_labels_list = df.is_legal_notification.values
