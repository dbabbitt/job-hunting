...
########################################################
## SGDClassifier is-preferred-qualification functions ##
########################################################
class IsPreferredQualificationSgdClassifier(PosSymbolSgdClassifier):
    ...
    def prepare_training_data(self, verbose=False):
        ...
        df.is_preferred_qualification = df.is_preferred_qualification.map(
            lambda x: {'True': 1, 'False': 0, True: 1, False: 0}.get(x, x)
        )
        if verbose:
            print(
                f'I have {df.shape[0]:,} hand-labeled',
                'preferred qualification htmls prepared'
            )
        train_data_list = df.navigable_parent.tolist()
        train_labels_list = df.is_preferred_qualification.values
