...
class TestHcMethods(unittest.TestCase):
    def setUp(self):
        ...
        self.ssgdcu.build_section_classifier(
            sampling_strategy_limit=None, verbose=True
        )
        self.scrfcu = SectionCRFClassifierUtilities(cu=cu, ha=ha, verbose=False)
        self.scrfcu.build_crf_with_pos_symbol_inputs(verbose=True)
    ...
    def test_get_feature_dict_list(self):
        ...
        self.assertEqual(self.hc.construct_feature_dict_list(child_tags_list, is_header_list, self.test_child_strs_list), feature_dict_list)
